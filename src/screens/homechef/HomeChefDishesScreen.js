...
export default function HomeChefDishesScreen({ route, navigation }) {
  ...
  const loadDishes = async () => {
    try {
      console.log(`🔍 جلب أطباق الشيف ${chefName} (ID: ${chefId})...`);
      ...
      console.log(`✅ تم جلب ${res.documents.length} طبق للشيف ${chefName}`);
      
      const dishesWithProvider = res.documents.map(d => ({
        ...d,
        providerName: chefName,
        providerId: chefId,
  ...
  return (
    <SafeAreaView style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle} numberOfLines={1}>أطباق {chefName}</Text>
...
const styles = StyleSheet.create({
  ...
  headerTitle: { fontSize: 18, fontWeight: 'bold', color: '#EF4444', flex: 1, textAlign: 'center' },
  list: { padding: 16 },
  dishCard: {
    flexDirection: 'row',
    backgroundColor: '#FFF',
  ...
  dishPrice: { fontSize: 14, fontWeight: 'bold', color: '#EF4444', marginTop: 2 },
