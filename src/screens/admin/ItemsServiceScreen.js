...
export default function ItemsServiceScreen({ navigation, route }) {
  ...
  useEffect(() => {
    loadItems();
    // تهيئة أسعار الخدمات الفرعية
    const initialPrices = {};
    subServices.forEach(sub => { initialPrices[sub] = ''; });
  ...
  const loadItems = async () => {
    setLoading(true);
    try {
      const response = await databases.listDocuments(DATABASE_ID, collectionName);
  ...
  const pickImage = async () => {
    try {
      ...
      if (!result.canceled) {
        ...
        setUploading(false);
        if (uploadResult.success) setImage(uploadResult.fileUrl);
  ...
  const resetForm = () => {
    ...
    setIsActive(true);
    const resetPrices = {};
    subServices.forEach(sub => { resetPrices[sub] = ''; });
  ...
  const openEditModal = (item) => {
    ...
    // تجهيز الأسعار للتعديل
    if (item.prices) {
      try {
        const prices = JSON.parse(item.prices);
        const priceMap = {};
        prices.forEach(p => { priceMap[p.subService] = p.price.toString(); });
  ...
  const handleSave = async () => {
    ...
    // التحقق من ملء جميع أسعار الخدمات الفرعية
    ...
    setUploading(true);
    try {
      // تجهيز بيانات الأسعار كـ JSON string
      const pricesArray = subServices.map(sub => ({
        subService: sub,
      ...
      const itemData = {
        ...
        isActive,
        serviceId,
        prices: pricesJson, // سنخزن الأسعار كنص في حقل واحد
      };

      if (editingItem) {
        await databases.updateDocument(DATABASE_ID, collectionName, editingItem.$id, itemData);
        Alert.alert('✅ تم', 'تم تحديث الصنف');
      } else {
        await databases.createDocument(DATABASE_ID, collectionName, ID.unique(), itemData);
  ...
  const handleDelete = (item) => {
    Alert.alert('حذف', `حذف ${item.name}؟`, [
      ...
      { text: 'حذف', style: 'destructive', onPress: async () => {
          await databases.deleteDocument(DATABASE_ID, collectionName, item.$id);
          loadItems();
      }},
    ]);
  };
  ...
  const toggleActive = async (item) => {
    await databases.updateDocument(DATABASE_ID, collectionName, item.$id, { isActive: !item.isActive });
    loadItems();
  };
    ...
    return (
      <View style={[styles.itemCard, !item.isActive && styles.itemCardDisabled]}>
        ...
        <View style={styles.actionButtons}>
          <TouchableOpacity style={[styles.actionButton, item.isActive ? styles.disableButton : styles.enableButton]} onPress={() => toggleActive(item)}>
  ...
  return (
    <SafeAreaView style={styles.container}>
      <View style={styles.header}>
        ...
        <TouchableOpacity onPress={() => { resetForm(); setModalVisible(true); }}><Ionicons name="add-circle" size={28} color="#4F46E5" /></TouchableOpacity>
      </View>

      <FlatList data={items} renderItem={renderItem} keyExtractor={item => item.$id} contentContainerStyle={styles.list}
        ListEmptyComponent={<View style={styles.emptyContainer}><Ionicons name="cube-outline" size={80} color="#E5E7EB" /><Text style={styles.emptyText}>لا توجد أصناف</Text></View>}
      />
          ...
          <ScrollView contentContainerStyle={styles.modalScrollContent}>
            <View style={styles.modalContent}>
              <View style={styles.modalHeader}>
                <Text style={styles.modalTitle}>{editingItem ? 'تعديل' : 'إضافة'} صنف</Text>
              ...
              <TextInput style={styles.input} value={itemName} onChangeText={setName} placeholder="مثال: بنطلون" />
              ...
              <TouchableOpacity style={styles.imagePicker} onPress={pickImage} disabled={uploading}>
                {itemImage ? <Image source={{ uri: itemImage }} style={styles.previewImage} /> : (
                  <View style={styles.imagePlaceholder}>
                    {uploading ? <ActivityIndicator color="#4F46E5" /> : <><Ionicons name="camera-outline" size={40} color="#9CA3AF" /><Text style={styles.imagePlaceholderText}>اختر صورة</Text></>}
                  </View>
                )}
              </TouchableOpacity>
              ...
              <TouchableOpacity style={[styles.saveButton, uploading && styles.disabled]} onPress={handleSave} disabled={uploading}>
                {uploading ? <ActivityIndicator color="#FFF" /> : <Text style={styles.saveButtonText}>{editingItem ? 'حفظ' : 'إضافة'}</Text>}
              </TouchableOpacity>
            </View>
          </ScrollView>
        </View>
...
const styles = StyleSheet.create({
  ...
  itemCard: { flexDirection: 'row', backgroundColor: '#FFF', borderRadius: 12, padding: 12, marginBottom: 12, borderWidth: 1, borderColor: '#E5E7EB', alignItems: 'center', justifyContent: 'space-between' },
  ...
  subServiceRow: { flexDirection: 'row', alignItems: 'center', marginBottom: 10, backgroundColor: '#F9FAFB', padding: 10, borderRadius: 8 },
  subServiceLabel: { flex: 2, fontSize: 14, color: '#1F2937' },
  priceInput: { flex: 1, backgroundColor: '#FFF', borderWidth: 1, borderColor: '#E5E7EB', borderRadius: 8, padding: 8, marginHorizontal: 8, fontSize: 14, textAlign: 'center' },
