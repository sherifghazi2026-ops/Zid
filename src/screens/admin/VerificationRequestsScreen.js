...
import { View, Text, StyleSheet, FlatList, TouchableOpacity, SafeAreaView, ActivityIndicator, RefreshControl, Image, Modal, Alert } from 'react-native';
  ...
  const loadRequests = async () => {
    try {
      const response = await databases.listDocuments(DATABASE_ID, USERS_COLLECTION_ID, [
        ...
        Query.or([Query.notEqual('verificationImage', null), Query.notEqual('verificationImage', '')]),
  ...
  const handleVerify = (userId) => {
    Alert.alert('تأكيد التوثيق', 'هل أنت متأكد من توثيق هذا التاجر؟', [
      ...
      { text: 'توثيق', onPress: async () => {
        ...
        if (result.success) { Alert.alert('✅ تم', 'تم توثيق التاجر'); loadRequests(); }
...
const styles = StyleSheet.create({
  ...
  verifyButton: { backgroundColor: '#10B981', paddingHorizontal: 20, paddingVertical: 10, borderRadius: 8 },
