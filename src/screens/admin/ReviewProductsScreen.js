...
export default function ReviewProductsScreen({ navigation }) {
  ...
  const handleApprove = (product) => {
    Alert.alert('موافقة', `الموافقة على المنتج "${product.name}"؟`, [
      { text: 'إلغاء' },
      {
        text: 'موافقة',
        onPress: async () => {
          const res = await approveProduct(product.$id);
          if (res.success) { Alert.alert('✅ تم', 'تمت الموافقة'); loadPending(); }
  ...
  const handleReject = () => {
    ...
    Alert.alert('رفض', `رفض "${selectedProduct?.name}"؟`, [
      { text: 'إلغاء' },
      {
        text: 'رفض',
        style: 'destructive',
  ...
  const renderProduct = ({ item }) => (
    <View style={styles.productCard}>
      <Image source={{ uri: item.imageUrl || 'https://via.placeholder.com/70' }} style={styles.productImage} />
      <View style={styles.productInfo}>
        <Text style={styles.productName}>{item.name}</Text>
        <Text style={styles.productPrice}>{item.price} ج</Text>
        <Text style={styles.productMerchant}>تاجر: {item.merchantName}</Text>
      ...
      <View style={styles.actions}>
        ...
        <TouchableOpacity style={[styles.actionButton, styles.rejectButton]} onPress={() => { setSelectedProduct(item); setRejectModal(true); }}>
  ...
  return (
    <SafeAreaView style={styles.container}>
      <View style={styles.header}>
        ...
        <TouchableOpacity onPress={loadPending}><Ionicons name="refresh" size={24} color="#4F46E5" /></TouchableOpacity>
      </View>
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            ...
            <View style={styles.modalButtons}>
              <TouchableOpacity style={[styles.modalButton, styles.cancelButton]} onPress={() => { setRejectModal(false); setRejectReason(''); }}>
                <Text style={styles.cancelButtonText}>إلغاء</Text>
              </TouchableOpacity>
              <TouchableOpacity style={[styles.modalButton, styles.confirmButton]} onPress={handleReject}>
...
const styles = StyleSheet.create({
  ...
  actionButton: { width: 36, height: 36, borderRadius: 18, justifyContent: 'center', alignItems: 'center', marginVertical: 4 },
