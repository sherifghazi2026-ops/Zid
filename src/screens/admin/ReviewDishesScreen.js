...
export default function ReviewDishesScreen({ navigation }) {
  ...
  const loadPendingDishes = async () => {
    try {
      ...
      dishesResponse.documents.forEach(dish => {
        if (dish.videoUrl) {
          initialStatus[dish.$id] = { isPlaying: false };
        }
      });
  ...
  const toggleVideo = async (dishId) => {
    const videoRef = videoRefs.current[dishId];
    if (!videoRef) return;

    const currentStatus = videoStatus[dishId] || { isPlaying: false };
    
    if (currentStatus.isPlaying) {
      await videoRef.pauseAsync();
      setVideoStatus(prev => ({ ...prev, [dishId]: { isPlaying: false } }));
    } else {
      // إيقاف أي فيديو شغال حالياً
      Object.keys(videoRefs.current).forEach(async (id) => {
        if (id !== dishId && videoStatus[id]?.isPlaying) {
          const ref = videoRefs.current[id];
          if (ref) {
            await ref.pauseAsync();
            setVideoStatus(prev => ({ ...prev, [id]: { isPlaying: false } }));
          }
        }
      });
      
      await videoRef.playAsync();
      setVideoStatus(prev => ({ ...prev, [dishId]: { isPlaying: true } }));
    }
  };

  const renderDishCard = ({ item }) => {
    const provider = providers[item.providerId] || { name: 'جاري التحميل...', type: '' };
    const isVideoPlaying = videoStatus[item.$id]?.isPlaying || false;
          ...
          <View style={styles.dishHeader}>
            ...
            {item.videoUrl && (
              <View style={styles.videoIndicator}>
                <Ionicons 
                  name={isVideoPlaying ? "videocam" : "videocam-outline"} 
                  size={18} 
                  color={isVideoPlaying ? "#EF4444" : "#6B7280"} 
                />
              </View>
            )}
          </View>
            ...
            <View style={styles.modalHeader}>
              ...
              <TouchableOpacity 
                onPress={() => {
                  ...
                  if (selectedDish?.videoUrl && videoRefs.current[selectedDish.$id]) {
                    videoRefs.current[selectedDish.$id].pauseAsync();
                  }
                  ...
                  <View style={styles.videoSection}>
                    ...
                    <View style={styles.videoContainer}>
                      <Video
                        ref={ref => {
                          if (ref) {
                            videoRefs.current[selectedDish.$id] = ref;
                          }
                        }}
                        source={{ uri: selectedDish.videoUrl }}
                        style={styles.video}
                        ...
                        onPlaybackStatusUpdate={(status) => {
                          if (status.didJustFinish) {
                            setVideoStatus(prev => ({ 
                              ...prev, 
                              [selectedDish.$id]: { isPlaying: false } 
                            }));
                          }
                        }}
                      />
                      ...
                      <TouchableOpacity 
                        ...
                        onPress={() => toggleVideo(selectedDish.$id)}
                      >
                        <Ionicons 
                          name={videoStatus[selectedDish.$id]?.isPlaying ? "pause" : "play"} 
                          size={50} 
                          color="#FFF" 
                        />
                      </TouchableOpacity>
