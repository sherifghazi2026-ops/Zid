...
import {
  View, Text, StyleSheet, FlatList, TouchableOpacity, SafeAreaView, Image, ActivityIndicator, RefreshControl,
} from 'react-native';
  ...
  const renderMerchant = ({ item }) => (
    <TouchableOpacity
      ...
      onPress={() => navigation.navigate('MerchantProductsScreen', {
        ...
        merchantName: item.name || item.full_name,
        merchantImage: item.image_url || item.avatar_url,
        serviceType: serviceType,
        serviceName: serviceName
      })}
    >
      <Image source={{ uri: item.image_url || item.avatar_url || 'https://via.placeholder.com/80' }} style={styles.merchantImage} />
      <View style={styles.merchantInfo}>
        <Text style={styles.merchantName}>{item.name || item.full_name}</Text>
        {item.description && <Text style={styles.merchantDesc} numberOfLines={2}>{item.description}</Text>}
        {item.delivery_fee !== undefined && <Text style={styles.deliveryInfo}>توصيل: {item.delivery_fee} ج</Text>}
      </View>
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <FlatList
        ...
        ListEmptyComponent={<View style={styles.emptyContainer}><Ionicons name="storefront-outline" size={80} color="#E5E7EB" /><Text style={styles.emptyText}>لا يوجد تجار متاحين</Text></View>}
      />
      <DynamicMongez screen="service" navigation={navigation} contextData={{ serviceId: serviceType, serviceName }} />
    </SafeAreaView>
  );
}
...
const styles = StyleSheet.create({
  ...
  list: { padding: 16 },
  merchantCard: { flexDirection: 'row', backgroundColor: '#FFF', borderRadius: 12, padding: 12, marginBottom: 12, borderWidth: 1, borderColor: '#E5E7EB', alignItems: 'center' },
