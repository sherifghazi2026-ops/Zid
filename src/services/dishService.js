...
export const deleteDish = async (dishId, image_urls = []) => {
  try {
    if (image_urls?.length) {
      ...
      for (const url of image_urls) await deleteFromImageKit(url).catch(e => console.log(e.message));
    }
    const { error } = await supabase.from(DISHES_COLLECTION_ID).delete().eq('id', dishId);
    if (error) throw error;
    return { success: true };
  } catch (error) {
    return { success: false, error: error.message };
